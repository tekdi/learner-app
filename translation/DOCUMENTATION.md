...
The original scripts hardcoded an absolute path belonging to a *different developer's machine*:

```js
'/home/ttpl-rt-132/Downloads/translation/...'   // not this machine → ENOENT
```

The scripts could not run as-is on anyone else's checkout. This copy instead derives the
repo root from the script's own location:

```js
const REPO_ROOT = path.resolve(__dirname, '..'); // <repo>/translation → <repo>
```

so it works on any checkout with no path edits.
...
The CSV columns are whatever `keyColumns` lists. A nested JSON path maps to them by depth —
the last segment is the `Key`, the preceding segments fill the module columns:
...
**In this repo the locale files are flat** — every key is a single top-level string, with
no nesting — so `keyColumns` is just `['Key']` and the CSV has one key column:
...
> `REPO_ROOT` is derived from the script's location (`path.resolve(__dirname, '..')`), so
> paths resolve automatically on any checkout — nothing machine-specific to edit. The only
> path you set per run is the import `csvPath` (where the client's returned sheet lives).
> There is no CLI parsing; keeping it simple makes the file easy to copy into another repo.
...
> ⚠️ **Match the `code` to the real filename.** Language filename codes vary widely
> between repos (e.g. Odiya is `odia` here but `odi`/`or` elsewhere; Marathi is `ma` here
> but `mr` elsewhere). Always confirm the exact filename the app loads and set `code`
> accordingly — a wrong code silently produces all-blank columns (export) or an orphan
> output file that never matches the real one (import).

---

## 9. Language codes & script ranges

Codes differ per repo — always match what the target repo actually uses. This repo
(`learner-app`) uses the codes in the **`learner-app`** column below.

| Language | `learner-app` (this repo) | Script (for validity check) |
|----------|---------------------------|-----------------------------|
| English | `en` | — (master) |
...
| Tamil | `ta` | Tamil `U+0B80–0BFF` |
| Gujarati | `gu` | Gujarati `U+0A80–0AFF` |
| Urdu | `ur` | Arabic `U+0600–06FF` (+ supplements) |

> **Marathi shares Devanagari with Hindi.** The script check cannot tell them apart (see
> §10), so a Marathi cell containing Hindi text will still pass. This is a known limitation,
> not a bug.
